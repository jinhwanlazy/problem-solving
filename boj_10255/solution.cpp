...
int solve() {
  ...
  vector<Seg2> bounds{Seg2(tl, bl), Seg2(bl, br), Seg2(br, tr), Seg2(tr, tl)};

  vector<int> inds;
  vector<Vec2> intersections;
  for (size_t i = 0; i < 4; ++i) {
    if (seg.intersects(bounds[i])) {
      inds.push_back(i);
      intersections.push_back(bounds[i].intersection(seg));
    }
  }
  ...
  if (inds.size() == 0) {
    ...
    if (seg.colinear(bounds[inds[0]]) && seg.overlap(bounds[inds[0]]).length() > 0) {
      return 4;
    }
    return 1;
  } else if (inds.size() == 2) {
    for (size_t i = 0; i < inds.size(); ++i) {
      if (seg.colinear(bounds[inds[i]]) && seg.overlap(bounds[inds[i]]).length() > 0) {
        return 4;
      }
    }
