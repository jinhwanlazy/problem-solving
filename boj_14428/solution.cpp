...
class SegTree {
 ...
 private:
  ...
  T init(const vector<U>& data, size_t left, size_t right, size_t node_idx) {
    if (left == right) {
      return tree_[node_idx] = transform(data[left], left);
    }
  ...
  T update(size_t idx,
    ...
    if (node_left == node_right) {
      return tree_[node_idx] = transform(new_value, node_left);
    }
    size_t mid = (node_left + node_right) / 2;
  ...
  template <typename U>
  static T transform(const U& v, size_t idx) {
    return {v, idx};
  }

  static T aggregate(const T& lhs, const T& rhs) { return min(lhs, rhs); }

  static T falseValue() { return {INF, -1}; }
};
...
int main() {
  ...
  SegTree<pair<int, size_t>> tree(xs);
