...
void draw(const vector<vector<Vec2>> shells, vector<Vec2>& res, int i) {
  if (i >= shells.size()) {
    return;
  }
  vector<Vec2> shell = shells[i];
  if (shell.empty()) {
    return;
  }
  if (i % 2 == 1) {
    reverse(shell.begin()+1, shell.end());
  }
  res.push_back(shell.front());
  draw(shells, res, i+1);
  for (int i = 1; i < shell.size(); ++i) {
    res.push_back(shell[i]);
  }
}
...
void solution() {
  ...
  auto shell = convexHull(points);
  set<Vec2> used(shell.begin(), shell.end());

  //for (int i = 0; i < shell.size(); ++i) {
    //cout << pointIndex[shell[i]] << ", ";
  //}
  //printf("\n");

  points.erase(remove_if(all(points),
        [&used](const Vec2& p) -> bool {
        return used.find(p) != used.end();
        }),
      points.end());
  sort(all(points));
  auto it = points.begin();
  for (const auto& p : shell) {
    while (it != points.end() && it->x < p.x) {
      cout << pointIndex[*it] << " ";
      it++;
    }
    cout << pointIndex[p] << " ";
