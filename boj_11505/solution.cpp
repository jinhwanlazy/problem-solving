...
class SegTree {
 ...
 protected:
  ...
  node_t query(size_t left,
    ...
    node_t res_right = query(left, right, nodeIdx * 2 + 1, mid + 1, nodeRight);
  ...
  node_t update(size_t idx,
    ...
    node_t res_right = update(idx, value, nodeIdx * 2 + 1, mid + 1, nodeRight);
    return tree_[nodeIdx] = aggregate(res_left, res_right);
  }
...
class RangeQuery : public SegTree<LL> {
 public:
  RangeQuery(const vector<LL>& data) : SegTree<LL>(data) { init(data); }

  LL defaultValue() const { return 1; }

  LL transform(const LL& i) const { return i; }

  LL aggregate(const LL& l, const LL& r) const { return (l * r) % MOD; }
};
