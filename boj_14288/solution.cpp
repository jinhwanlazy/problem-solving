...
int eulerTraversal(const vector<vector<int>>& tree, int root, vector<pair<int, int>>& ranges) {
  vector<int> tmp;
...
void solve() {
  ...
  vector<int> data(l, 0);
  RangeQuery upQuery(data);
  RangeQuery downQuery(data);
  ...
  while (M--) {
    ...
    switch (queryType) {
      case 1: {
        ...
        auto [l, r] = ranges[i];
        if (down) {
          downQuery.update(l, downQuery.query(l, l) + w);
          downQuery.update(r, downQuery.query(r, r) - w);
        }
        else {
          upQuery.update(l, upQuery.query(l, l) + w);
        }
      } break;
      case 2: {
        int i;
        ...
        i--;
        auto [l, r] = ranges[i];
        int ans = upQuery.query(l, r) + downQuery.query(0, l);
        cout << ans << "\n";
      } break;
      case 3: {
        down ^= 1;
