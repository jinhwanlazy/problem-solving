...
void solve() {
  ...
  auto getAngle = [&centroid](const Vec2& p) {
    return atan2(p.y - centroid.y, p.x - centroid.x);
  };

  sort(points.begin(), points.end(), [&getAngle](const Vec2& p, const Vec2& q) -> bool {
    return getAngle(p) < getAngle(q);
  });

  size_t startIdx = min_element(points.begin(), points.end(), [](const Vec2& p, const Vec2& q) -> bool {
    if (p.x != q.x)
      return p.x < q.x;
    return p.y < q.y;
  }) - points.begin();
  for (size_t i = 0; i < points.size(); ++i) {
    const auto& p = points[(i + startIdx) % points.size()];
    cout << p << "\n";
  }
}
